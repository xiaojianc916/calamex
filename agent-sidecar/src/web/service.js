...
import { aiWebFetchInputSchema, aiWebFetchPayloadSchema, aiWebSearchInputSchema, aiWebSearchPayloadSchema, aiWebSearchResultSchema, } from './types.js';
// ---------------------------------------------------------------------------
// 常量
// ---------------------------------------------------------------------------
...
const CONTEXT_SIGNATURE_PATTERNS = [
...
const ensureMcpSuccess = (value, fallbackMessage) => {
 ...
 */
export const disposeWebService = async () => {
    const pending = sharedBundlePromise;
    sharedBundlePromise = null;
    if (!pending)
        return;
    await pending.then((bundle) => bundle.disconnectAll().catch(() => undefined), () => undefined);
};
    ...
    try {
        return await Promise.race([
            ...
            new Promise((_, reject) => {
                timer = setTimeout(() => reject(new Error(`Tavily 工具调用超时(${timeoutMs}ms)：${label}`)), timeoutMs);
            }),
        ]);
    }
    finally {
...
const executeTavilyTool = async (toolName, args) => {
    const bundle = await getSharedBundle();
    const tool = bundle.tools[toolName];
    if (!isExecutableTool(tool)) {
        throw new Error(`未找到官方 Tavily MCP 工具：${toolName}`);
    }
    const invoke = (payload) => withTimeout(tool.execute(payload), TAVILY_TOOL_TIMEOUT_MS, toolName);
    try {
...
const clipToByteLimit = (value, maxBytes) => {
    if (maxBytes <= 0)
        return { text: '', bytes: 0 };
...
const isDocsHost = (host, path) => host.startsWith('docs.') ||
    host.startsWith('developer.') ||
    host.includes('.docs.') ||
    path.startsWith('/docs') ||
    path.startsWith('/doc/');
const isGithubHost = (host) => matchesDomain(host, 'github.com') || host.endsWith('.github.io');
const isForumHost = (host) => {
...
const isBlogHost = (host, path) => host.startsWith('blog.') || host.endsWith('.blog') || path.startsWith('/blog');
/** 优先级：official > docs > github > forum > blog > unknown */
const classifySourceType = (rawUrl) => {
...
// Tavily 文本解析
// ---------------------------------------------------------------------------
const isFieldPrefixLine = (line) => TAVILY_FIELD_PREFIXES.some((prefix) => line.startsWith(prefix));
const buildSearchResult = (acc) => {
...
const safeParseSearchResult = (candidate) => {
    const parsed = aiWebSearchResultSchema.safeParse(candidate);
    return parsed.success ? parsed.data : null;
};
const parseSearchText = (text, input) => {
...
const parseExtractText = (text, input) => {
    const lines = text.split(/\r?\n/gu);
    let title = '';
    let rawContent = '';
...
const buildSearchArgs = (input) => {
    const days = toRecencyDays(input.recency);
    return {
        query: input.query, // schema 已经 trim
        topic: input.intent === 'release-notes' ? 'news' : 'general',
        max_results: Math.max(MIN_TAVILY_SEARCH_RESULTS, Math.min(MAX_WEB_SEARCH_RESULTS, input.maxResults)),
...
// 公共 API
// ---------------------------------------------------------------------------
export const searchWeb = async (rawInput) => {
...
export const fetchWeb = async (rawInput) => {
