...
const isPrivateIpv6 = (host) => {
    ...
    if (/^f[cd][0-9a-f]{0,2}:/u.test(host))
        return true;
    // Link-local fe80::/10 → 首段 fe80–febf
    if (/^fe[89ab][0-9a-f]{0,2}:/u.test(host))
        return true;
    // Multicast ff00::/8
    if (/^ff[0-9a-f]{0,2}:/u.test(host))
        return true;
    return false;
};
/** 拦截十六进制 / 十进制整数 / 八进制前导零等 IPv4 非规范写法。 */
const looksLikeObfuscatedIpv4 = (host) => {
    if (/^0x[0-9a-f]+$/iu.test(host))
        return true; // 0x7f000001
    if (/^\d{8,10}$/u.test(host))
        return true; // 2130706433
 ...
 */
export const isAllowedPublicHttpUrl = (value) => {
    if (value.length > AI_WEB_LIMITS.URL_MAX)
        return false;
    let url;
    try {
        url = new URL(value);
    ...
    if (looksLikeObfuscatedIpv4(host))
        return false;
