...
const unwrapModelToolInput = (value) => {
...
const removeNullishFields = (value, fields) => {
    if (!isObjectRecord(value)) {
        return value;
    }
    let normalized = null;
...
const currentTimeNormalizedInputSchema = z.preprocess((value) => removeNullishFields(unwrapModelToolInput(value), ['timezone']), currentTimeBaseInputSchema);
const convertTimeNormalizedInputSchema = z.preprocess((value) => removeNullishFields(unwrapModelToolInput(value), ['source_timezone', 'target_timezone', 'date']), convertTimeBaseInputSchema);
...
const normalizeUnicodeText = (value) => value.normalize('NFKC').trim();
const weekdayFormatterCache = new Map();
const createWeekdayFormatter = (timezone) => {
...
const resolveLocalTimezone = (configuredTimezone) => {
...
const resolveTimezone = (value, fallbackTimezone) => {
    const normalized = value ? normalizeUnicodeText(value) : '';
    return normalized ? validateTimezone(normalized) : fallbackTimezone;
};
const parseCurrentTimeInput = (value) => currentTimeNormalizedInputSchema.parse(value);
const parseConvertTimeInput = (value) => convertTimeNormalizedInputSchema.parse(value);
const NANOSECONDS_PER_MINUTE = 60_000_000_000;
const formatZonedDateTime = (zonedDateTime) => zonedDateTime.toString({ smallestUnit: 'second', timeZoneName: 'never' });
const isDaylightSavingTime = (zonedDateTime) => {
...
const createTimeSnapshot = (zonedDateTime) => ({
...
const formatTimeDifference = (offsetMinutes) => {
    if (offsetMinutes === 0) {
        return '0h';
    }
...
export const createMastraTimeTools = (options = {}) => {
    const now = options.now ?? (() => new Date());
    const localTimezone = resolveLocalTimezone(options.localTimezone ?? process.env.AGENT_MCP_LOCAL_TIMEZONE);
    const currentZonedDateTime = (timezone) => Temporal.Instant.fromEpochMilliseconds(now().getTime()).toZonedDateTimeISO(timezone);
    return {
        ...
        convert_time: createTool({
            ...
            execute: async (inputData) => {
                ...
                const timeDifferenceMinutes = Math.round((targetZonedDateTime.offsetNanoseconds - sourceZonedDateTime.offsetNanoseconds) / NANOSECONDS_PER_MINUTE);
                return {
                    source,
                    target,
