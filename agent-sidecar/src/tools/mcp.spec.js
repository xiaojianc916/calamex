...
import { createMastraMcpClientBundle, getMcpRuntimeStatus, loadMcpServerConfigs, MCP_SERVER_NAMES, } from './mcp.js';
...
const createMockStdioConfig = (name) => ({
    name,
    transportType: 'stdio',
...
function assertStdioConfig(config, name) {
    assert.ok(config, `应找到 ${name} MCP 配置`);
    assert.equal(config.transportType, 'stdio');
}
function assertHttpConfig(config, name) {
    assert.ok(config, `应找到 ${name} MCP 配置`);
...
describe('MCP gateway warm pool', () => {
    ...
    it('lists all MCP gateway catalogs with one tool call', async () => {
        ...
        assert.deepEqual(catalog.catalogs.map((item) => item.serverName), MCP_SERVER_NAMES);
        assert.deepEqual(Object.keys(catalog), ['profile', 'catalogs', 'errors']);
        assert.equal(catalog.catalogs.every((item) => item.tools.length === 1), true);
        await pool.disconnectAll();
    });
    it('coalesces concurrent all-server catalog calls', async () => {
        ...
        const catalogs = await Promise.all(Array.from({ length: 10 }, () => executeList({}, makeToolExecutionContext())));
        assert.equal(createBundleCalls, MCP_SERVER_NAMES.length);
    ...
    it('returns the full MCP catalog to the model without truncation', async () => {
        const pool = createMcpGatewayWarmPool({
            createBundle: async (options) => {
                ...
                const tools = Object.fromEntries(Array.from({ length: 30 }, (_, index) => [
                    ...
                    memory_lookup: {
                        ...
                        execute: async (inputData) => {
                            ...
                            if ('context' in record) {
                                ...
                                return { key: record.context.key, value: 'ok' };
                            }
                            rawAttemptCount += 1;
    ...
    it('reports readonly profile filtering separately from an empty MCP server', async () => {
        ...
        await assert.rejects(async () => executeCall({
            serverName: 'git',
            toolName: 'commit',
            arguments: { message: 'test' },
        }, makeToolExecutionContext()), /当前 readonly profile 不允许使用 git 的任何 MCP tool/u);
        await pool.disconnectAll();
    });
    it('filters local file primitive duplicates out of MCP catalog and calls', async () => {
        ...
        const catalog = catalogCollection.catalogs.find((item) => item.serverName === 'git');
        ...
        }, makeToolExecutionContext()), /file primitives 接管/u);
        await pool.disconnectAll();
    });
});
describe('MCP sidecar config', () => {
    ...
    it('skips Git MCP when Windows git.exe cannot be resolved', () => {
        ...
        assert.equal(loaded.errors.some((error) => error.includes('AGENT_MCP_GIT_EXECUTABLE_PATH')), true);
    });
    ...
    it('builds a Mastra-ready MCP bundle from the official MCPClient and keeps healthy tools when one configured server closes', async () => {
        ...
        try {
            ...
            assert.equal(bundle.errors.some((error) => error.includes('git')), true);
        }
        finally {
