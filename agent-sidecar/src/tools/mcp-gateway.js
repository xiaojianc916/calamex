...
import { MCP_SERVER_NAMES, } from './mcp.js';
const DEFAULT_MCP_GATEWAY_MAX_WARM = 4;
const DEFAULT_MCP_GATEWAY_TTL_IDLE_MS = 5 * 60_000;
const DEFAULT_MCP_GATEWAY_PINNED_SERVERS = ['memory'];
const DEFAULT_MCP_GATEWAY_PINNED_SERVERS_IGNORE_WORKSPACE = ['memory'];
...
const GIT_NATIVE_FILE_DUPLICATE_TOOL_PATTERN = /(?:^|[_*-])(?:read_file|write_file|edit_file|create_directory|move_file|delete_file|grep)(?:$|[_*-])/iu;
const PROBE_NATIVE_FILE_DUPLICATE_TOOL_PATTERN = /(?:^|[_*-])(?:grep|search_code|search_files|extract_code|read_file|list_files)(?:$|[_*-])/iu;
export const MCP_GATEWAY_TOOL_NAMES = ['mcp_list_tools', 'mcp_call_tool'];
...
const createEmptyGatewayBundle = () => ({
    tools: {},
    configs: [],
    errors: [],
...
const unwrapGatewayToolInput = (value) => {
...
const createToolDescription = (tool, toolName) => {
    const raw = getToolDescription(tool).replace(/\s+/gu, ' ').trim();
    return raw || `(no description for ${toolName})`;
};
const normalizeMcpServerToolPrefix = (serverName) => serverName.replace(/-/gu, '_');
const isNativeFilePrimitiveDuplicateTool = (serverName, toolName) => {
    switch (serverName) {
        case 'git':
...
const filterNativeFilePrimitiveDuplicates = (serverName, tools) => {
    const filteredTools = {};
...
const SIDE_EFFECT_FREE_SERVERS = new Set([
    'sequential-thinking',
    'context7',
]);
...
export const readMcpToolAnnotations = (tool) => {
    const record = toRecord(tool);
    if (!record) {
        return undefined;
    }
...
export const resolveMcpToolCapability = (serverName, annotations) => {
...
export const requiresMcpToolApproval = (serverName, annotations) => resolveMcpToolCapability(serverName, annotations) !== 'readonly';
const filterMcpToolsForProfile = (serverName, tools, profile) => {
...
const resolveMcpGatewayTool = (tools, serverName, toolName) => {
...
const executeMcpGatewayToolWithTimeout = async (tool, args, timeoutMs) => {
    ...
    return await withTimeout(invoke(), timeoutMs, () => new Error(`MCP tool 调用超时（${timeoutMs}ms）。`));
};
const createPoolKey = (workspaceRootPath, serverName, pinnedIgnoreWorkspace) => {
...
const createCatalogKey = (serverName, profile) => `${serverName}::${profile}`;
const readErrors = (bundle) => bundle.errors ?? [];
const readConfigs = (bundle) => bundle.configs ?? [];
const createUnavailableReason = (serverName, bundle, profile, filteredToolCount) => {
    const rawToolCount = Object.keys(bundle.tools).length;
    const nativeFilteredToolCount = Object.keys(filterNativeFilePrimitiveDuplicates(serverName, bundle.tools)).length;
...
const createCatalogFromBundle = (serverName, profile, bundle) => {
...
const createToolUnavailableError = (serverName, toolName, bundle, profile, availableTools) => {
...
export class McpGatewayMetricBuffer {
    metrics = [];
    listener = null;
    droppedCount = 0;
    emit(metric) {
...
export class McpGatewayWarmPool {
    ...
    ttlIdleMs;
    pinnedServers;
    pinnedServersIgnoreWorkspace;
    callTimeoutMs;
    now;
    entries = new Map();
    catalog = new Map();
    toolCallCounts = new Map();
    listAllInflight = new Map();
    constructor(options) {
        ...
        this.pinnedServersIgnoreWorkspace = new Set(options.pinnedServersIgnoreWorkspace ?? DEFAULT_MCP_GATEWAY_PINNED_SERVERS_IGNORE_WORKSPACE);
    ...
    async listAllToolsUncached(input) {
        const catalogs = [];
        const errors = [];
    ...
    async callTool(input) {
        ...
        return await this.withServer(input, async (bundle) => {
            ...
            if (!resolvedTool) {
                throw createToolUnavailableError(input.serverName, input.toolName, bundle, input.profile, Object.keys(tools));
            }
            const frequencyKey = `${input.serverName}/${resolvedTool.name}`;
            try {
                const result = await executeMcpGatewayToolWithTimeout(resolvedTool.tool, input.arguments, this.callTimeoutMs);
    ...
    cacheCatalogVariants(serverName, bundle) {
    ...
    emitCatalogMetric(input, cacheHit, startedAt, catalog) {
        input.metricSink?.emit({
    ...
    async evictExpired() {
        const now = this.now();
        const expiredEntries = [...this.entries.values()].filter((entry) => (Boolean(entry.bundle)
            && entry.activeCount === 0
    ...
    async evictOverflow() {
        const candidates = [...this.entries.values()]
            .filter((entry) => (Boolean(entry.bundle)
            && entry.activeCount === 0
            && !this.pinnedServers.has(entry.serverName)))
...
export const createMcpGatewayWarmPool = (options) => new McpGatewayWarmPool(options);
