...
import { PLAN_ORCHESTRATION_WORKFLOW_ID, createPlanOrchestrationWorkflow } from './plan/orchestration-workflow.js';
...
export class MastraRuntime extends MastraRuntimeApproval {
     ...
     */
    buildPlanOrchestrationWorkflow(modelConfig) {
        const workflow = createPlanOrchestrationWorkflow(createMastraPlanOrchestrationDeps({
        ...
        const mastra = new Mastra({
            workflows: { [PLAN_ORCHESTRATION_WORKFLOW_ID]: workflow },
            storage: this.storage,
        });
        // workflow 值 as never 会使 getWorkflowById 的参数联合塌缩为 never，
        // 给实参加 as never（纯编译期，运行时传真实 id）解除。
        return mastra.getWorkflowById(PLAN_ORCHESTRATION_WORKFLOW_ID);
    }
    async restoreCheckpoint(input, options = {}) {
        ...
        const modelConfig = resolveMastraModelConfig(this.readModelConfig, 'modelConfig' in input ? input.modelConfig : undefined);
        if (!modelConfig) {
            return createErrorResponse(sessionId, 'AI 模型未配置：请先在应用设置中完成 Mastra 模型配置。', events, options);
        }
        try {
            ...
            if (!snapshot) {
                ...
                return createErrorResponse(sessionId, 'Mastra 回滚恢复失败：未找到可恢复的 checkpoint。', events, options);
            }
            if (snapshot.status === 'running') {
                ...
                return createErrorResponse(sessionId, 'Mastra 回滚恢复失败：当前 run 仍在执行，暂时不能回滚。', events, options);
            }
            ...
            if (!systemPrompt) {
                ...
                return createErrorResponse(sessionId, 'Mastra 回滚恢复失败：checkpoint 缺少可恢复的系统提示词。', events, options);
            }
            const workspaceRootPath = resolveWorkspaceRootPathFromSnapshot(snapshot);
            const { bundle: mcpBundle, tools: mastraTools, hasTools, workspace, browser, } = await loadMastraMcpTools(this.mcpGatewayPool, workspaceRootPath, this.loggerRef, [], 'write', {
                mode: 'agent',
            ...
            try {
                ...
                return createErrorResponse(sessionId, `Mastra 回滚恢复失败：${normalizeMastraError(error)}`, events, options);
            }
            finally {
