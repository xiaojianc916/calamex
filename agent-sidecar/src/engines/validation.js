...
export class MastraRuntimeValidation extends MastraRuntimePlan {
    async preparePlanAgentRun(input, sessionId, events, options, runIdPrefix, missingPlanMessage) {
        ...
        if (!planId || !Number.isInteger(planVersion) || Number(planVersion) <= 0) {
            return {
                ok: false,
                response: createErrorResponse(sessionId, missingPlanMessage, events, options),
            };
        }
        ...
        if (!modelConfig) {
            return {
                ok: false,
                response: createErrorResponse(sessionId, 'AI 模型未配置：请在 Node sidecar 环境设置 AGENT_SIDECAR_MODEL 与 AGENT_SIDECAR_API_KEY。', events, options),
            };
        }
        ...
        const toolBundle = await loadMastraMcpTools(this.mcpGatewayPool, input.workspaceRootPath, this.loggerRef, input.context ?? [], 'readonly', memoryInput);
    ...
    async finalizePlanAgentRun(sessionId, requestedRunId, toolBundle) {
    ...
    async validatePlan(input, options = {}) {
        const sessionId = input.sessionId ?? createSessionId('mastra-plan-validate');
        const events = [];
        const prepared = await this.preparePlanAgentRun(input, sessionId, events, options, 'mastra-plan-validator-run', '计划验证需要 planId 和 planVersion。');
        if (!prepared.ok) {
            return prepared.response;
        }
        const { planId, version, modelConfig, record, workflow, workflowEvents, toolBundle, requestedRunId, memory, agentMemory, payloadEventSink, } = prepared;
        const { tools: mastraTools, hasTools, toolStats, mcpGatewayMetrics, workspace, browser, } = toolBundle;
        try {
            return await runWithDeepSeekReasoningContext({
                ...
                const toolChoice = hasTools || Boolean(workspace) || Boolean(browser) ? 'auto' : 'none';
                ...
                const mastraMessages = [{ role: 'user', content: prompt }];
                ...
                if (!report) {
                    return createErrorResponse(sessionId, 'Validator 没有返回有效验证报告。', events, options);
                }
            ...
            return createErrorResponse(sessionId, `Validator 执行失败：${normalizeMastraError(error)}`, events, options);
        }
        finally {
            await this.finalizePlanAgentRun(sessionId, requestedRunId, toolBundle);
        }
    }
    async replanPlan(input, options = {}) {
        const sessionId = input.sessionId ?? createSessionId('mastra-plan-replan');
        const events = [];
        const prepared = await this.preparePlanAgentRun(input, sessionId, events, options, 'mastra-plan-replanner-run', '重新规划需要 planId 和 planVersion。');
        if (!prepared.ok) {
            return prepared.response;
        }
        const { planId, version, modelConfig, record, workflow, workflowEvents, toolBundle, requestedRunId, memory, agentMemory, payloadEventSink, } = prepared;
        ...
        try {
            return await runWithDeepSeekReasoningContext({
                ...
                if (!delta) {
                    return createErrorResponse(sessionId, 'Replanner 没有返回有效 delta plan。', events, options);
                }
                const nextPlan = applyAgentPlanDelta(record.plan, delta);
                if (!nextPlan) {
                    return createErrorResponse(sessionId, 'Replanner 生成的 delta plan 无法应用到当前计划。', events, options);
                }
            ...
            return createErrorResponse(sessionId, `Replanner 执行失败：${normalizeMastraError(error)}`, events, options);
        }
        finally {
            await this.finalizePlanAgentRun(sessionId, requestedRunId, toolBundle);
        }
