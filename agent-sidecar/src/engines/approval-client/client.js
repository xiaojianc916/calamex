...
export class MastraRuntimeApproval extends MastraRuntimeExecution {
    async resolveApproval(input, options = {}) {
        ...
        try {
            return await runWithDeepSeekReasoningContext({
                ...
                const resumeOptions = {
                    ...
                    ...(approvalContext.memory && approvalContext.systemPrompt ? {
                        ...
                        }, approvalContext.systemPrompt, approvalContext.memory, approvalContext.approvedPlanRecord),
                    } : {}),
                };
                ...
                const streamSummary = await this.consumeTextStream(pending.agent, pending.bundle, sessionId, stream, events, options, createRuntimeEvent, pending.workspace, pending.browser, workflowTracker ?? undefined);
                ...
                if (streamSummary.streamErrorMessage) {
                    ...
                    return createErrorResponse(sessionId, `Mastra Approval 执行失败：${streamSummary.streamErrorMessage}`, events, options);
                }
            ...
            return createErrorResponse(sessionId, `Mastra Approval 执行失败：${normalizeMastraError(error)}`, events, options);
        }
        finally {
            if (shouldDisconnectBundle) {
                evictDeepSeekReasoningByPrefix(createDeepSeekReasoningRunPrefix(sessionId, decodedRequest.runId));
                streamCleanup?.();
                await pending.bundle.disconnectAll();
                await destroyMastraWorkspace(pending.workspace);
                await destroyMastraBrowser(pending.browser);
     ...
     */
    resolveApprovalWorkflowTracker(input) {
