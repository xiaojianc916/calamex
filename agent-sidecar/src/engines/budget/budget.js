...
export const createJsonToolModelOutput = (value) => ({
    type: 'json',
    value,
});
...
export const isZodSchemaLike = (value) => {
...
const toolInputSchemaCache = new WeakMap();
const computeToolInputSchemaForBudget = (schema) => {
...
export const convertToolInputSchemaForBudget = (schema) => {
    if (!schema) {
        return EMPTY_TOOL_PARAMETERS;
    }
...
export const createProviderToolBudgetShape = (name, tool) => {
...
export const countProviderToolSchemaChars = (tools) => countJsonChars(Object.entries(tools).map(([name, tool]) => createProviderToolBudgetShape(name, tool)));
export const createAcontextTokenEventDraft = (input) => {
    const messagesText = stringifyForBudget(input.messages);
    const toolsText = stringifyForBudget(Object.entries(input.tools).map(([name, tool]) => createProviderToolBudgetShape(name, tool)));
...
export const createAcontextProviderPayloadEventDraft = (stats, requestIndex) => ({
...
export const createDeepSeekPayloadEventSink = (events, options) => {
    const pending = [];
    let runtimeEventFactory = null;
    let requestIndex = 0;
