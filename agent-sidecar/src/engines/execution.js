...
export class MastraRuntimeExecution extends MastraRuntimeValidation {
    async execute(input, options = {}) {
        ...
        if (!planId || !planStepId || !Number.isInteger(planVersion) || Number(planVersion) <= 0) {
            return createErrorResponse(sessionId, 'Agent 执行需要已批准计划的 planId、planVersion 和 planStepId。', events, options);
        }
        ...
        try {
            ...
            return createErrorResponse(sessionId, `Plan 执行门禁失败：${normalizeMastraError(error)}`, events, options);
        }
        const memoryInput = {
            ...normalizedInput,
        ...
        const { bundle: mcpBundle, tools: mastraTools, hasTools, toolStats, mcpGatewayMetrics, workspace, browser, } = await loadMastraMcpTools(this.mcpGatewayPool, normalizedInput.workspaceRootPath, this.loggerRef, normalizedInput.context ?? [], 'write', memoryInput);
        ...
        try {
            return await runWithDeepSeekReasoningContext({
                ...
                const stream = await executionHandle.agent.stream(mastraMessages, {
                    maxSteps: hasAgentTools ? AGENT_EXECUTION_MAX_STEPS : 1,
                    toolChoice,
                    memory,
                    ...(options.context?.signal ? { abortSignal: options.context.signal } : {}),
                    runId: requestedRunId,
                    requestContext: createExecutionRequestContext(memoryInput, systemPrompt, memory, approvedPlanRecord),
                });
                ...
                const streamSummary = await this.consumeTextStream(executionHandle.agent, mcpBundle, sessionId, stream, events, options, createCheckpointEvent, workspace, browser, {
                    planId,
                    version: Number(planVersion),
                    stepId: planStepId,
                });
