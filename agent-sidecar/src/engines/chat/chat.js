...
export class MastraRuntimeChat extends MastraRuntimeBase {
    async runTextMode(input, mode, sessionPrefix, options = {}) {
        ...
        if (hasImageAttachmentParts(normalizedInput.context) && !isVisionModelId(modelConfig.modelId)) {
            return createErrorResponse(sessionId, '当前模型不支持图片理解，请切换支持视觉输入的模型后再发送图片。', events, options);
        }
        const { bundle: mcpBundle, tools: mastraTools, hasTools, toolStats, mcpGatewayMetrics, workspace, browser, } = executionPlan.useTools
            ? await loadMastraMcpTools(this.mcpGatewayPool, normalizedInput.workspaceRootPath, this.loggerRef, normalizedInput.context ?? [], normalizedInput.mode === 'agent' ? 'write' : 'readonly', normalizedInput)
            : {
        ...
        const memory = executionPlan.useMemory
            ? createMastraMemoryReference(createMastraMemoryScope(normalizedInput, sessionId, executionPlan.useTools ? { resourceScope: 'session' } : {}))
            : null;
        ...
        try {
            return await runWithDeepSeekReasoningContext({
                ...
                const streamSummary = await this.consumeTextStream(agent, mcpBundle, sessionId, stream, events, options, createRuntimeEvent, workspace, browser);
                shouldDisconnectBundle = streamSummary.releaseResources;
                if (streamSummary.streamErrorMessage) {
                    return createErrorResponse(sessionId, `Mastra Agent 执行失败：${streamSummary.streamErrorMessage}`, events, options);
                }
                ...
                const result = streamSummary.visibleText.trim().length > 0
                    ...
                    : 'Agent 已完成。';
                const doneEvent = createDoneOutputEvent(result, streamSummary.doneTokenSnapshot);
            ...
            return createErrorResponse(sessionId, `Mastra Agent 执行失败：${normalizeMastraError(error)}`, events, options);
        }
        finally {
