...
const inferModelProviderLabel = (modelId) => {
...
// Shared sections
// -----------------------------------------------------------------------------
const buildIdentitySection = (modelId) => {
...
const buildContextSection = (context = []) => {
    ...
    const blocks = visibleContext.map((reference, index) => {
        const truncated = truncateModelOutputText(reference.contentPreview, CONTEXT_REFERENCE_PREVIEW_MAX_CHARS);
        const isTruncated = truncated.truncated === true;
        return [
...
// Public API
// -----------------------------------------------------------------------------
export const buildSystemPrompt = (input, modelId = UNSPECIFIED_MODEL_LABEL) => {
    ...
    const goalSection = trimmedGoal
        ...
        : '';
    const extraSystemSection = systemMessages.length > 0
...
export const extractVisibleAgentResultText = (result) => {
