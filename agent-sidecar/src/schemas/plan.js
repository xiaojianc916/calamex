 ...
 *   **独立**版本号（两套持久化分别演化）。
 */
export const AGENT_PLAN_RECORD_SCHEMA_VERSION = 1;
// ----------------------------------------------------------------------
...
export const agentPlanRecordSchema = z.object({
    ...
    return value.rejectionReason === null;
}, { message: 'rejectionReason 仅且必须在 status="rejected" 时设置', path: ['rejectionReason'] }).refine((value) => {
    // errorMessage 仅在 'failed' 状态出现。
    if (value.status === 'failed') {
        return value.errorMessage !== null;
    }
    return value.errorMessage === null;
}, { message: 'errorMessage 仅且必须在 status="failed" 时设置', path: ['errorMessage'] }).refine((value) => {
    ...
    return value.approvedAt === null;
}, { message: 'approvedAt 必须与 status 一致（approved/executing/completed/failed 时必填）', path: ['approvedAt'] }).refine((value) => {
