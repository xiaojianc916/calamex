 ...
 */
export const AGENT_PLAN_WORKFLOW_EVENT_SCHEMA_VERSION = 1;
// ----------------------------------------------------------------------
// JSON value
// ----------------------------------------------------------------------
/**
...
}).refine((value) => !(value.approved && value.rejected), { message: 'approval 不能同时 approved 与 rejected', path: ['approved'] }).refine((value) => value.rejected ? value.reason !== null : true, { message: 'approval.rejected 时必须填写 reason', path: ['reason'] });
...
}).refine((value) => value.executionCursor <= value.stepIds.length, { message: 'executionCursor 不能超过 stepIds.length', path: ['executionCursor'] }).refine((value) => {
