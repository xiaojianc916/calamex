...
test("totalTokens 优先，size 取上下文窗口", () => {
	const update = toUsageUpdate({ totalTokens: 1200, inputTokens: 800, outputTokens: 400 }, 128_000)
	assert.deepEqual(update, { sessionUpdate: "usage_update", used: 1200, size: 128_000 })
})

test("缺 totalTokens 时回退为 prompt+completion", () => {
	const update = toUsageUpdate({ inputTokens: 800, outputTokens: 400 }, 200_000)
