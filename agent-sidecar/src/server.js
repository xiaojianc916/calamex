...
import { z } from 'zod';
import { AGENT_RUNTIME_OUTPUT_EVENT_TYPES, toAgentSidecarResponse, toAgentUiEvent, } from './engines/contracts/runtime-contracts.js';
...
import { aiWebFetchInputSchema, aiWebFetchPayloadSchema, aiWebSearchInputSchema, aiWebSearchPayloadSchema, } from './web/types.js';
...
import { logWarmupResult, scheduleBackgroundWarmup, warmupLlmConnection, } from './http/warmup.js';
...
const isOrchestrationWorkflowDisabled = () => {
...
const isRuntimeOutputEvent = (value) => {
    if (!value || typeof value !== 'object') {
        return false;
    }
    const candidateType = value.type;
    return (typeof candidateType === 'string'
        && AGENT_RUNTIME_OUTPUT_EVENT_TYPES.includes(candidateType));
};
...
const extractOrchestrationAgentEvent = (chunk) => {
 ...
 */
const rollbackStepSchema = z.preprocess((value) => (typeof value === 'string' ? [value] : value), z.array(requiredNonEmptyStringSchema).min(1));
...
const normalizeSidecarToken = (value) => {
...
const isAuthorizedSidecarRequest = (request, token) => {
    if (!token) {
        return true;
    }
...
const toAgentInput = (payload, mode) => {
...
const handlePost = async (request, response, handler) => {
    try {
        const body = await readBody(request);
        writeJson(response, 200, toValidatedSidecarResponse(await handler(body, createRuntimeRunOptions(request))));
    }
    catch (error) {
...
const handleRuntimeResponse = async (request, response, handler) => {
    try {
        writeJson(response, 200, toValidatedSidecarResponse(await handler(createRuntimeRunOptions(request))));
    }
    catch (error) {
        writeJson(response, 400, {
            error: error instanceof Error ? error.message : String(error),
...
const handlePlainPost = async (request, response, handler) => {
    try {
        const body = await readBody(request);
...
const handleWarmupPost = async (request, response) => {
    try {
...
const writeNdjsonFrame = (response, payload) => {
...
const createRuntimeRunOptions = (request, onEvent) => {
...
const toValidatedSidecarResponse = (response) => {
    const payload = toAgentSidecarResponse(response);
    agentSidecarResponseSchema.parse(payload);
    return payload;
};
const handlePostStream = async (request, response, handler) => {
    try {
        const body = await readBody(request);
...
// Server
// -----------------------------------------------------------------------
export const createAgentSidecarServer = (options = {}) => {
    ...
    const orchestrationRunTimers = new Map();
    const forgetOrchestrationRun = (runId) => {
    ...
    const rememberOrchestrationRun = (runId, run) => {
    ...
    return createServer((request, response) => {
        ...
        if (request.method === 'GET' && parsedUrl.pathname.startsWith('/agent/plan/')) {
            ...
            void handleRuntimeResponse(request, response, async (options) => runtime.getPlan(payload.data, options));
            return;
        }
        ...
        if (request.method === 'POST' && url === '/web/search') {
            void handlePlainPost(request, response, async (body) => aiWebSearchPayloadSchema.parse(await searchWeb(aiWebSearchInputSchema.parse(body))));
            return;
        }
        if (request.method === 'POST' && url === '/web/fetch') {
            void handlePlainPost(request, response, async (body) => aiWebFetchPayloadSchema.parse(await fetchWeb(aiWebFetchInputSchema.parse(body))));
            return;
        }
...
const logProcessEvent = (event, error) => {
