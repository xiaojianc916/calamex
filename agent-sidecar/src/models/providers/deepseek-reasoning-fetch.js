...
export const deepseekReasoningContext = new AsyncLocalStorage();
...
const REASONING_DEBUG_ENABLED = process.env.AGENT_SIDECAR_DEEPSEEK_REASONING_DEBUG === '1';
// -----------------------------------------------------------------------------
// Generic helpers
const isRecord = (value) => toRecord(value) !== null;
const logReasoningDebug = (event, fields) => {
    if (!REASONING_DEBUG_ENABLED)
        return;
    console.info('[deepseek-reasoning]', { event, ...fields });
};
const logReasoningWarning = (event, fields, error) => {
...
// Key construction
// -----------------------------------------------------------------------------
const normalizeToolCallIds = (toolCallIds) => [...new Set(toolCallIds
        .map((id) => id.trim())
        .filter((id) => id.length > 0))].sort();
export const createDeepSeekReasoningKey = (context, toolCallIds) => {
...
export const createDeepSeekReasoningRunPrefix = (sessionId, runId) => `${sessionId}::${runId}::`;
const createDeepSeekReasoningContextPrefix = (context) => createDeepSeekReasoningRunPrefix(context?.sessionId ?? 'anon', context?.runId ?? 'anon');
// -----------------------------------------------------------------------------
// Store lifecycle
// -----------------------------------------------------------------------------
...
const enforceStoreCapacity = () => {
    if (reasoningStore.size <= REASONING_STORE_MAX_ENTRIES)
        return;
...
export const evictDeepSeekReasoningByPrefix = (prefix) => {
    for (const key of reasoningStore.keys()) {
...
// Public API
// -----------------------------------------------------------------------------
export const runWithDeepSeekReasoningContext = async (context, task) => deepseekReasoningContext.run(context, task);
export const clearDeepSeekReasoningStoreForTest = () => {
    reasoningStore.clear();
};
export const setDeepSeekReasoningForTest = (context, toolCallIds, reasoning) => {
...
// -----------------------------------------------------------------------------
const getToolCallIds = (toolCalls) => {
...
const hasReasoningContent = (message) => typeof message.reasoning_content === 'string'
    && message.reasoning_content.length > 0;
const storeReasoning = (context, toolCallIds, reasoning) => {
    const normalized = normalizeToolCallIds(toolCallIds);
...
const findStoredReasoning = (context, toolCallIds) => {
    const normalized = normalizeToolCallIds(toolCallIds);
    const now = Date.now();
    const isFresh = (entry) => now - entry.createdAt <= REASONING_TTL_MS;
    const exact = reasoningStore.get(createDeepSeekReasoningKey(context, normalized));
    if (exact && isFresh(exact))
        return exact;
...
const normalizeMessageContentFields = (body) => {
 ...
 */
const injectReasoningIntoMessages = (body, context) => {
    const messages = body.messages;
    if (!Array.isArray(messages))
        return false;
    let changed = false;
 ...
 */
const stripReasoningFromMessages = (body) => {
    const messages = body.messages;
    if (!Array.isArray(messages))
        return false;
    let changed = false;
...
// Telemetry
// -----------------------------------------------------------------------------
const readMessageContentCharCount = (message) => countJsonChars(message.content);
const readToolCount = (tools) => Array.isArray(tools) ? tools.length : 0;
const createRequestPayloadStats = (body, thinkingMode, reasoningInjected, reasoningStripped) => {
...
const emitRequestPayloadStats = (context, body, thinkingMode, reasoningInjected, reasoningStripped) => {
    try {
        context?.onRequestPayload?.(createRequestPayloadStats(body, thinkingMode, reasoningInjected, reasoningStripped));
    }
    catch (error) {
...
// Streaming capture
// -----------------------------------------------------------------------------
const captureReasoningFromJson = (body, context) => {
...
const createStreamingCaptureState = () => {
    let state;
...
const finalizeStreamingCapture = (context, decoder, state) => {
    if (state.finalized)
        return;
...
// -----------------------------------------------------------------------------
const shouldHandleRequest = (body) => isRecord(body) && Array.isArray(body.messages);
const readRequestJsonBody = async (input, init) => {
    ...
    if (input instanceof Request && rawBody === undefined) {
        const parsed = JSON.parse(await input.clone().text());
        return shouldHandleRequest(parsed) ? parsed : null;
    }
    return null;
};
const createRequestWithJsonBody = (input, init, body) => {
...
const prepareOutboundRequest = async (input, init) => {
    ...
    try {
        ...
        emitRequestPayloadStats(context, body, thinkingMode, reasoningInjected, reasoningStripped);
        return changed
...
// Response handling
// -----------------------------------------------------------------------------
const responseInitFrom = (response) => ({
    status: response.status,
    statusText: response.statusText,
    headers: new Headers(response.headers),
});
const captureNonStreamingResponse = async (response, context) => {
    const text = await response.text();
    try {
        if (text.trim().length > 0) {
            captureReasoningFromJson(JSON.parse(text), context);
        }
    }
    catch (error) {
...
const captureStreamingResponse = (response, context) => {
    if (!response.body)
        return response;
...
// -----------------------------------------------------------------------------
export const deepseekReasoningFetch = async (input, init) => {
...
export const encodeSseLineForTest = (line) => textEncoder.encode(line);
