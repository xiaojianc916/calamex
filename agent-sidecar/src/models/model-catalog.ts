 ...
 *
 * Sidecar counterpart of the frontend `src/lib/model-catalog.ts`. The two live
 * in separate packages/builds and cannot share code, so this is an intentional
 * mirror with identical robustness semantics.
 *
 * Why this exists:
 ...
 *   breaking the build. Callers (e.g. capability resolution) then fall back to
 *   their hand-written seed values — i.e. today's behaviour. This makes the
...
/**
 * Minimal structural view of the subset of `tokenlens` we rely on. Kept loose
 * on purpose: every member is optional so a version mismatch is handled at
 * runtime via `typeof` guards rather than at the import boundary.
 */
