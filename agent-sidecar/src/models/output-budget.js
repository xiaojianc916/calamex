...
const hasIntlSegmenter = () => typeof Intl !== 'undefined'
    && typeof Intl.Segmenter === 'function';
const getGraphemeSegmenter = (locale) => {
...
// ---- Option resolution -------------------------------------------------
const toBoundedInteger = (value, fallback, min) => {
...
// ---- Notice formatting -------------------------------------------------
const formatTruncationNotice = (kept, total, locale) => {
    if (locale.toLowerCase().startsWith('zh')) {
        return `[内容已截断：显示前 ${kept} / ${total} 字符。]`;
    }
    return `[Content truncated: showing ${kept} of ${total} characters.]`;
};
const formatDepthExceededNotice = (locale) => locale.toLowerCase().startsWith('zh')
    ? '[内容已省略：超过最大结构深度。]'
    : '[Content omitted: maximum structure depth exceeded.]';
const formatCycleNotice = (locale) => locale.toLowerCase().startsWith('zh')
    ? '[内容已省略：检测到循环引用。]'
    : '[Content omitted: circular reference detected.]';
// ---- Public helpers ----------------------------------------------------
export const countModelOutputChars = (value, locale = DEFAULT_LOCALE) => segmentGraphemes(value, locale).length;
export const truncateModelOutputText = (value, maxChars, options = {}) => {
...
const describeOpaqueObject = (value) => {
...
const compactValue = (value, options, depth, seen) => {
    ...
    if (typeof value !== 'object')
        return String(value);
    if (seen.has(value))
        return formatCycleNotice(options.locale);
    if (depth >= options.maxDepth)
        return formatDepthExceededNotice(options.locale);
    seen.add(value);
    try {
...
// ---- Public entry point ------------------------------------------------
const stringifyCompactValue = (value) => {
    try {
...
export const compactModelOutput = (value, options) => {
    const resolved = resolveOptions(options);
    const compacted = compactValue(value, resolved, 0, new WeakSet());
...
export const clearGraphemeSegmenterCacheForTest = () => {
