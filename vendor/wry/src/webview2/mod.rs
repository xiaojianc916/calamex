...
#[cfg(feature = "visual-hosting")]
#[inline]
const fn get_x_lparam(lparam: LPARAM) -> i32 {
  (loword(lparam.0 as usize) as i16) as i32
}

#[cfg(feature = "visual-hosting")]
#[inline]
const fn get_y_lparam(lparam: LPARAM) -> i32 {
  (hiword(lparam.0 as usize) as i16) as i32
}

#[cfg(feature = "visual-hosting")]
#[inline]
const fn get_keystate_wparam(wparam: WPARAM) -> u32 {
  loword(wparam.0) as u32
}

#[cfg(feature = "visual-hosting")]
#[inline]
const fn get_xbutton_wparam(wparam: WPARAM) -> u32 {
  hiword(wparam.0) as u32
}

#[cfg(feature = "visual-hosting")]
#[inline]
const fn get_wheel_delta_wparam(wparam: WPARAM) -> u32 {
  ((hiword(wparam.0) as i16) as i32) as u32
}

#[cfg(feature = "visual-hosting")]
#[inline]
const fn get_pointer_id_wparam(wparam: WPARAM) -> u32 {
  loword(wparam.0) as u32
}

#[cfg(feature = "visual-hosting")]
#[inline]
unsafe fn screen_to_client(hwnd: HWND, point: &mut POINT) -> bool {
...
pub(crate) struct InnerWebView {
  ...
  is_child: bool,
  #[cfg(feature = "visual-hosting")]
  visual_host: Option<visual_host::VisualHost>,
...
#[cfg(feature = "visual-hosting")]
struct VisualParentSubclassData {
  host: visual_host::VisualHost,
  is_tracking_mouse: bool,
  is_capturing_mouse: bool,
  pointer_ids_starting_in_webview: HashSet<u32>,
}

#[cfg(feature = "visual-hosting")]
impl VisualParentSubclassData {
  fn new(host: visual_host::VisualHost) -> Self {
    Self {
      host,
      is_tracking_mouse: false,
      is_capturing_mouse: false,
      pointer_ids_starting_in_webview: HashSet::new(),
    }
  }
  ...
  unsafe fn on_mouse_message(
    ...
    let is_mouse_in_webview = InnerWebView::point_in_client(hwnd, point);
    if is_mouse_in_webview || msg == WM_MOUSELEAVE || self.is_capturing_mouse {
      let mouse_data = match msg {
        WM_MOUSEWHEEL | WM_MOUSEHWHEEL => get_wheel_delta_wparam(wparam),
        WM_XBUTTONDBLCLK | WM_XBUTTONDOWN | WM_XBUTTONUP => get_xbutton_wparam(wparam),
        _ => 0,
      };

      match msg {
        WM_MOUSEMOVE if !self.is_tracking_mouse => {
          InnerWebView::track_mouse_events(hwnd, TME_LEAVE);
      ...
      let _ = self.host.comp_controller.SendMouseInput(
        COREWEBVIEW2_MOUSE_EVENT_KIND(msg as i32),
        COREWEBVIEW2_MOUSE_EVENT_VIRTUAL_KEYS(get_keystate_wparam(wparam) as i32),
        mouse_data,
        point,
      );
      true
    } else if msg == WM_MOUSEMOVE && self.is_tracking_mouse {
      self.is_tracking_mouse = false;
      InnerWebView::track_mouse_events(hwnd, TME_LEAVE | TME_CANCEL);
      self.on_mouse_message(hwnd, WM_MOUSELEAVE, WPARAM(0), LPARAM(0));
      false
    } else {
      false
    }
  ...
  unsafe fn on_pointer_message(&mut self, hwnd: HWND, msg: u32, wparam: WPARAM) -> bool {
    let pointer_id = get_pointer_id_wparam(wparam);
    let mut pointer_info = POINTER_INFO::default();
    if GetPointerInfo(pointer_id, &mut pointer_info).is_err() {
      return false;
    }
    ...
    let pointer_started_in_webview = self.pointer_ids_starting_in_webview.contains(&pointer_id);
    if InnerWebView::point_in_client(hwnd, point) || pointer_started_in_webview {
      ...
      let Ok(webview_pointer_info) = self.host.create_pointer_info(pointer_id) else {
        return false;
      };
      let _ = self.host.comp_controller.SendPointerInput(
        COREWEBVIEW2_POINTER_EVENT_KIND(msg as i32),
        &webview_pointer_info,
      );
      true
    } else {
      false
...
impl InnerWebView {
  ...
  fn new_in_hwnd(
    ...
    #[cfg(feature = "visual-hosting")]
    let use_visual_hosting = attributes.visual_hosting && !is_child;
    #[cfg(not(feature = "visual-hosting"))]
    let use_visual_hosting = false;

    let hwnd = if use_visual_hosting {
      parent
    } else {
    ...
    #[cfg(feature = "visual-hosting")]
    let visual_host = if use_visual_hosting {
      let PhysicalSize { width, height } = Self::parent_bounds(parent)?;
      Some(unsafe {
        visual_host::VisualHost::create(
          parent,
          &env,
    ...
    #[cfg(feature = "visual-hosting")]
    let controller = if let Some(visual_host) = &visual_host {
      visual_host.controller.clone()
    } else {
  ...
  fn init_webview(
    ...
    env: &ICoreWebView2Environment,
    controller: &ICoreWebView2Controller,
    #[cfg(feature = "visual-hosting")] visual_host: Option<&visual_host::VisualHost>,
    pl_attrs: super::PlatformSpecificWebViewAttributes,
    is_child: bool,
    ...
    if let Some(visual_host) = visual_host {
      let comp_controller = visual_host.comp_controller.clone();
      let host_hwnd = visual_host.hwnd;
      unsafe {
        comp_controller.add_CursorChanged(
          &CursorChangedEventHandler::create(Box::new(move |sender, _| {
            ...
            let _ = SetClassLongPtrW(host_hwnd, GCLP_HCURSOR, cursor.0 as _);
            let _ = if cursor.0.is_null() {
              SetCursor(None)
            } else {
  ...
  #[cfg(feature = "visual-hosting")]
  unsafe fn point_in_client(hwnd: HWND, point: POINT) -> bool {
    let mut client_rect = RECT::default();
  ...
  #[cfg(feature = "visual-hosting")]
  unsafe fn track_mouse_events(hwnd: HWND, mouse_tracking_flags: TRACKMOUSEEVENT_FLAGS) {
  ...
  unsafe fn handle_windowed_resize(controller: &ICoreWebView2Controller, hwnd: HWND) {
    ...
    let mut controller_hwnd = HWND::default();
    if controller.ParentWindow(&mut controller_hwnd).is_ok() {
      let _ = SetWindowPos(
        controller_hwnd,
        None,
        0,
  ...
  #[cfg(feature = "visual-hosting")]
  unsafe fn handle_visual_resize(host: &visual_host::VisualHost, hwnd: HWND) {
    let Ok(PhysicalSize { width, height }) = Self::parent_bounds(hwnd) else {
      return;
    };
  ...
  #[cfg(feature = "visual-hosting")]
  unsafe fn destroy_parent_subclass_data(hwnd: HWND, dwrefdata: usize) {
  ...
  unsafe extern "system" fn parent_subclass_proc(
    ...
    match msg {
      ...
      | WM_POINTERCAPTURECHANGED => {
        if let ParentSubclassData::Visual(state) = data {
          if state.on_pointer_message(hwnd, msg, wparam) {
            return LRESULT(0);
          }
        }
      }
      ...
      msg if msg == WM_DESTROY || msg == PARENT_DESTROY_MESSAGE => {
        #[cfg(feature = "visual-hosting")]
        {
          Self::destroy_parent_subclass_data(hwnd, dwrefdata);
        }
        #[cfg(not(feature = "visual-hosting"))]
        {
          drop(Box::from_raw(dwrefdata as *mut ParentSubclassData));
  ...
  #[inline]
  unsafe fn attach_parent_subclass(parent: HWND, data: ParentSubclassData) {
    let _ = SetWindowSubclass(
      parent,
      Some(Self::parent_subclass_proc),
      PARENT_SUBCLASS_ID as _,
...
impl InnerWebView {
  ...
  pub fn set_visible(&self, visible: bool) -> Result<()> {
    unsafe {
      #[cfg(feature = "visual-hosting")]
      if self.visual_host.is_none() || self.is_child {
        let _ = ShowWindow(
          self.hwnd,
          match visible {
            true => SW_SHOW,
