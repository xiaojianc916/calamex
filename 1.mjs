...
const SNAPSHOT_RS_PATH = "src-tauri/src/ai/edit/history/snapshot.rs"
const BLOB_STORE_RS_PATH = "src-tauri/src/ai/edit/history/blob_store.rs"

// 用尚未被本脚本改写过的 snapshot.rs 探测仓库实际使用的换行符风格
// （Windows checkout 常见 CRLF）。blob_store.rs 的整体重写也复用同一风格，
// 避免同一个仓库里出现两种混用的换行符。所有锚点匹配统一先归一化成 LF，
// 与磁盘上文件到底是 CRLF 还是 LF 无关，从根上消除这一类"锚点找不到"的假阴性。
function detectLineEnding(filePath) {
	const sample = readFileSync(path.join(REPO_ROOT, filePath), "utf8")
	return sample.includes("\r\n") ? "\r\n" : "\n"
}

const LINE_ENDING = detectLineEnding(SNAPSHOT_RS_PATH)

function toRepoLineEnding(lfContent) {
	return LINE_ENDING === "\r\n" ? lfContent.replace(/\n/g, "\r\n") : lfContent
}
...
function applyReplacements(filePath, replacements) {
	...
	const finalContent = toRepoLineEnding(content)
	...
	writeFileSync(absolute, finalContent, "utf8")
	console.log(`[written] ${filePath}：已应用 ${replacements.length} 处替换。`)
}

function overwriteFile(filePath, lfContent) {
	const absolute = path.join(REPO_ROOT, filePath)
	const finalContent = toRepoLineEnding(lfContent)
	if (DRY_RUN) {
		console.log(`[dry-run] ${filePath}：将整体重写（${finalContent.length} 字节），未写入。`)
		return
	}
	writeFileSync(absolute, finalContent, "utf8")
...
overwriteFile(BLOB_STORE_RS_PATH, BLOB_STORE_RS)

// ---------------------------------------------------------------------------
// 2) snapshot.rs：11 处精确替换
// ---------------------------------------------------------------------------

applyReplacements(SNAPSHOT_RS_PATH, [
	{
