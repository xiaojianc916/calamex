#!/usr/bin/env node
// patch6.mjs — ADR-0015 阶段2：把 Kimi / Codex provisioner 做成自包含（ADDITIVE，零行为变化）
//
// 用法（仓库根目录）：  node patch6.mjs
// 行为：
//   1) 覆盖 src-tauri/src/acp/provisioner.rs —— 完整内容从本脚本尾部的块注释中提取
//      （块注释规避一切 JS 字符串转义；标记串由 concat 拼出，正文里不会提前出现）。
//   2) 外科手术式放宽 src-tauri/src/acp/launch.rs 三个基元为 pub(super)（幂等、可重复运行）：
//        resolve_node_executable / path_to_string / env_or_user_env
//   运行后：旧（launch.rs 内 Kimi/Codex 自由函数）与新（provisioner.rs 内联实现）并存，
//   cargo 应全绿；patch #7 再删旧。
//
// 仅用 node 内置模块，无第三方依赖。
...
const provPath = path.join(repoRoot, "src-tauri", "src", "acp", "provisioner.rs")
const launchPath = path.join(repoRoot, "src-tauri", "src", "acp", "launch.rs")

// 从本脚本尾部的块注释提取 provisioner.rs 内容。标记串用 concat 拼接，
// 故「连续的标记文本」只在真正的尾部注释里出现一次，绝不会匹配到这段代码本身。
function readEmbeddedProvisioner() {
	const tag = "PROVISIONER_RS"
	...
	if (i < 0 || j < 0 || j <= i) {
		throw new Error("无法在脚本尾部定位 provisioner.rs 内容标记。")
	}
	return selfSrc.slice(i + start.length, j).replace(/\r\n/g, "\n").trim() + "\n"
}

function writeProvisioner() {
	const content = readEmbeddedProvisioner()
	// 落地前 sanity-check：确认关键锚点齐全，避免写入残缺文件。
	const anchors = [
		"pub trait ExternalAgentProvisioner",
		"impl ExternalAgentProvisioner for KimiProvisioner",
		"fn ensure_kimi_managed_config()",
		"pub fn provisioner_for(",
		"mod tests {",
	]
	for (const a of anchors) {
		if (!content.includes(a)) throw new Error(`provisioner.rs 内容缺少锚点：${a}`)
	}
	const prev = fs.existsSync(provPath) ? fs.readFileSync(provPath, "utf8") : ""
	const useCrlf = prev.includes("\r\n")
	const out = useCrlf ? content.replace(/\n/g, "\r\n") : content
	if (prev === out) {
		console.log("provisioner.rs 已是目标内容，跳过。")
		return
	}
	fs.writeFileSync(provPath, out)
	console.log(`已写入 provisioner.rs（${useCrlf ? "CRLF" : "LF"}，${content.length} 字符）。`)
}

function widenLaunch() {
	const raw = fs.readFileSync(launchPath, "utf8")
	const usedCrlf = raw.includes("\r\n")
	let text = raw.replace(/\r\n/g, "\n")
	// 每条 needle 都带开括号 `(`，从而只命中「定义」而不命中调用点或测试名
	// （如 fn path_to_string_roundtrips_simple_path( 不会被匹配）。
	const edits = [
		["fn resolve_node_executable(", "pub(super) fn resolve_node_executable("],
		["fn path_to_string(", "pub(super) fn path_to_string("],
		["fn env_or_user_env(", "pub(super) fn env_or_user_env("],
	]
	let changed = 0
	for (const [needle, replacement] of edits) {
		if (text.includes(replacement)) {
			console.log("launch.rs 已放宽，跳过：" + replacement)
			continue
		}
		const count = text.split(needle).length - 1
		if (count !== 1) {
			throw new Error(`launch.rs 需精确匹配 1 次「${needle}」，实际 ${count} 次；已中止未写入。`)
		}
		text = text.replace(needle, replacement)
		changed++
	}
	if (changed === 0) {
		console.log("launch.rs 三处可见性均已就绪，无需修改。")
		return
	}
	const out = usedCrlf ? text.replace(/\n/g, "\r\n") : text
	fs.writeFileSync(launchPath, out)
	console.log(`已放宽 launch.rs ${changed} 处函数可见性为 pub(super)（${usedCrlf ? "CRLF" : "LF"}）。`)
}

try {
	writeProvisioner()
	widenLaunch()
	console.log("\npatch6 完成。请在 src-tauri/ 下运行：cargo clippy --all-targets && cargo test")
} catch (err) {
	console.error("patch6 失败：" + err.message)
	process.exitCode = 1
}

/*=== PROVISIONER_RS_START ===
