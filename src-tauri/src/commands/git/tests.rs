...
fn init_git_repository_does_not_reuse_parent_repository() -> Result<(), String> {
    ...
    assert_eq!(
        status.repository_root_path.as_deref(),
        Some(expected.to_string_lossy().to_string().as_str())
    );
    Ok(())
}
...
fn stage_git_paths_and_unstage_git_paths_round_trip() -> Result<(), String> {
    ...
    let s = stage_git_paths(GitPathOperationRequest {
        ...
        paths: vec!["src/app.sh".into()],
    })?;
    assert_eq!(s.staged_count, 1);
    let s = unstage_git_paths(GitPathOperationRequest {
        repository_root_path: root.to_string_lossy().to_string(),
        paths: vec!["src/app.sh".into()],
    })?;
...
fn checkout_git_branch_switches_between_branches() -> Result<(), String> {
    ...
    let initial_branch = initial
        .head_branch_name
        .clone()
        .ok_or_else(|| "缺少初始分支名".to_string())?;
    // 新建并切换到 feature 分支，再切回初始分支，验证 HEAD 原子写入后分支正确。
    create_git_branch(GitBranchCreateRequest {
        repository_root_path: root.to_string_lossy().to_string(),
        branch_name: "feature/switch".into(),
        checkout: true,
    })?;
    let status = checkout_git_branch(GitBranchCheckoutRequest {
        repository_root_path: root.to_string_lossy().to_string(),
        branch_name: initial_branch.clone(),
    })?;
    assert_eq!(
        status.head_branch_name.as_deref(),
...
fn get_git_repository_status_reports_renamed_path_correctly() -> Result<(), String> {
    ...
    let renamed = status
        .files
        .iter()
        .find(|f| f.relative_path == "src/new_name.sh")
