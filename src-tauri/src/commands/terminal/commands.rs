...
use super::state::{
    ...
    complete_session_run_state, drain_active_terminal_runs, get_active_terminal_run_handle,
    ...
    set_session_geometry, set_session_state, set_terminal_snapshot, should_recreate_terminal_session,
...
pub fn dispatch_script_to_terminal(
    ...
    // 各自维护其运行/交互态，互不影响。
    set_session_state(&terminal_state, &payload.session_id, TerminalState::SwitchingToRun);
        ...
        && let Err(error) = transition_terminal_state(&app, TerminalState::SwitchingToRun)
    {
        ...
        complete_session_run_state(&terminal_state, &payload.session_id);
        return Err(error);
    }
