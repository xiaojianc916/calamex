...
pub fn shutdown_all_terminal_sessions(state: &TerminalSessionState) -> Result<(), String> {
    // 孤儿收割任务的停止已由进程级 CancellationToken 统一驱动（run_exit_cleanup 在调用本函数前
    // 已取消令牌，收割任务随即在 select! 中退出），故此处不再单独置位关停标志，仅负责收口 PTY。
...
/// 启动孤儿会话收割任务：周期性回收「长时间无前端心跳 + 无活动运行」的交互会话并终止其 PTY，
...
pub fn spawn_orphan_terminal_session_reaper(app: AppHandle, state: TerminalSessionState) {
    // 统一关停管线：观察进程级 CancellationToken。应用退出时 run_exit_cleanup 取消令牌，本任务
    // 在 select! 中即时命中 cancelled() 退出，替代此前的 AtomicBool 轮询 + sleep（最坏要睡满一个
    // 巡检周期才感知退出）。巡检由 tokio::time::interval 驱动，与 SSH 连接池清理任务同构。
    //
    // 收割体 reap_idle_orphan_terminal_sessions 为同步状态存取（std Mutex），仅在 await 之间整体
    // 执行、绝不跨 await 持有任何 MutexGuard，故在异步任务中执行安全（不阻塞执行器、无 Send 隐患）。
    let shutdown = crate::lifecycle::shutdown_token();
    tauri::async_runtime::spawn(async move {
        let mut interval = tokio::time::interval(ORPHAN_SESSION_REAP_POLL);
        interval.tick().await; // 跳过首个立即 tick，保持与原首轮间隔一致
        loop {
            tokio::select! {
                _ = interval.tick() => {
