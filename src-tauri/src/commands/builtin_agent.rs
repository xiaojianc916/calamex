...
pub async fn acp_prompt(
    ...
    let outcome: Result<AgentExternalChatResultPayload, crate::acp::AcpClientError> = async {
        ...
        // 模型目录经宿主 env 注入子进程（Zed 范式，见 acp/launch.rs 的 build_builtin_agent_env），
        // 边车启动时自省声明，不再经会话级 _meta 下发；外部 agent（Kimi/Codex）凭据自管，同样无需 meta。
        let acp_session_id = host
            .ensure_session(thread_id, workspace_root_path)
            .await?;
