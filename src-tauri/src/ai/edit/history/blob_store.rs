...
//! （`remove_blob`）都通过调用方传入的 `fjall::OwnedWriteBatch` 入队，随调用方的批次
...
pub fn store_blob(batch: &mut OwnedWriteBatch, blobs: &Keyspace, content: &[u8]) -> String {
...
/// 迁移前遗留的旧格式 key 过滤在外。
pub fn remove_blob(blobs: &Keyspace, batch: &mut OwnedWriteBatch, blob_key: &str) -> Result<u64, String> {
    validate_blob_key(blob_key)?;
