...
const CACHE_TTL_FAILED_SECS: i64 = 10 * 60;
...
fn parse_favicon_host(request: &Request<Vec<u8>>) -> Option<String> {
    let authority = request
        ...
        .unwrap_or_default();
    if !authority.is_empty() && !is_local_authority(authority) {
        return None;
    }

    let mut raw_host = request.uri().path().trim_matches('/').trim().to_lowercase();

    // 某些 WebView 会把 authority 折叠进 path，例如 "//localhost/example.com"。
    if authority.is_empty() {
        for prefix in ["localhost/", "127.0.0.1/", "[::1]/"] {
            if let Some(stripped) = raw_host.strip_prefix(prefix) {
                raw_host = stripped.to_string();
                break;
            }
        }
    }
...
#[cfg(test)]
mod tests {
    use super::parse_favicon_host;
    use tauri::http::Request;

    #[test]
    fn parse_host_accepts_standard_local_authority_uri() {
        let request = Request::builder()
            .uri("favicon://localhost/www.bbc.com")
            .body(Vec::new())
            .expect("request should build");

        let host = parse_favicon_host(&request);
        assert_eq!(host.as_deref(), Some("www.bbc.com"));
    }

    #[test]
    fn parse_host_accepts_path_with_embedded_localhost_prefix() {
        let request = Request::builder()
            .uri("/localhost/www.espn.com")
            .body(Vec::new())
            .expect("request should build");

        let host = parse_favicon_host(&request);
        assert_eq!(host.as_deref(), Some("www.espn.com"));
    }

    #[test]
    fn parse_host_rejects_non_local_authority() {
        let request = Request::builder()
            .uri("favicon://evil.example/www.bbc.com")
            .body(Vec::new())
            .expect("request should build");
...
async fn read_cache_entry(cache_root: &Path, host: &str) -> CacheLookup {
    ...
    if meta.status == "failed" {
        // 兼容历史版本写入的超长失败缓存（24h）：按当前策略视为过期并立即重试。
        if meta.expires_at - now_unix_secs() > CACHE_TTL_FAILED_SECS {
            let _ = fs::remove_file(&meta_path).await;
            let _ = fs::remove_file(&icon_path).await;
            return CacheLookup::Miss;
        }
