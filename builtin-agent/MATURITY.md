...
summary:
  ...
  - Runtime resolution is closed end-to-end. builtin_agent/mod.rs resolves both the sidecar root and the Node executable with a "bundled-first -> system fallback" strategy through the shared commands::shell_tools::bundled_resource_roots() helper, which is also reused by commands/lsp/discovery.rs.
  - Writable runtime state (builtin-agent.log and NODE_COMPILE_CACHE) is relocated to the user-writable LOCALAPPDATA/com.xiaojianc.Calamex/builtin-agent directory, so a read-only install dir (Program Files) no longer breaks logging or the compile cache.
resolved_gaps:
