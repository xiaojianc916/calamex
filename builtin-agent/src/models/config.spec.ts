...
test('createMastraModelConfigFromEnv：环境变量 baseUrl 使用同一套校验', () => {
  assert.throws(
    () => createMastraModelConfigFromEnv({
      BUILTIN_AGENT_API_KEY: 'test-key',
      BUILTIN_AGENT_MODEL: 'deepseek/deepseek-v4-pro',
      BUILTIN_AGENT_BASE_URL: 'file:///tmp/llm',
    }),
    /仅允许 http\/https 协议/u,
  );
});
...
test('background model config：显式环境变量可覆盖后台模型', () => {
  ...
  const observer = createMastraObserverModelConfig(baseModel, {
    BUILTIN_AGENT_OBSERVER_MODEL: 'deepseek/deepseek-v4-pro',
  });
  const reflector = createMastraReflectorModelConfig(baseModel, {
    BUILTIN_AGENT_REFLECTOR_MODEL: 'deepseek/deepseek-v4-pro',
  });
