// d1s4e-useAiAssistant-spec-acp-native.mjs
// 用法：node d1s4e-useAiAssistant-spec-acp-native.mjs  然后 pnpm typecheck && pnpm test useAiAssistant
// 作用：把 src/composables/ai/useAiAssistant.spec.ts 整体重写为 ACP-native 单一发送管线规格。
...
const rewrite = (rel, raw) => {
  const abs = resolve(ROOT, rel);
  if (!existsSync(abs)) throw new Error('[rewrite] 目标文件不存在: ' + rel);
  const eol = detectEol(readFileSync(abs, 'utf8'));
  writeFileSync(abs, fromLf(toLf(raw), eol), 'utf8');
  console.log('[rewrite] ' + rel + ' (' + raw.length + ' chars, eol=' + (eol === '\r\n' ? 'CRLF' : 'LF') + ')');
};

// 注意：以下 SPEC 内容刻意不含反引号 / 模板插值 / 反斜杠，保证可安全作为反引号字面量嵌入。
const SPEC = `import { createPinia, setActivePinia } from 'pinia';
...
rewrite('src/composables/ai/useAiAssistant.spec.ts', SPEC);
console.log('[done] useAiAssistant.spec.ts 已重写为 ACP-native 单一管线规格。');