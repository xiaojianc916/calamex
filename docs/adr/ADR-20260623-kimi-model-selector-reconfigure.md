...
### 阶段三（已落地，Option 1：单一后端感知选择器 + 退役 modes-as-model）

用户在澄清后锁定 **Option 1（纯 ACP 实时切换、无后端改动、最干净）**。本阶段把上面「后续
阶段」里规划的前端收敛真正落地，且**全程不动后端**（不新增命令、不改 Rust，复用阶段二已
seed 的候选池 + 既有 config_options 链路）。

- **单一后端感知选择器**：模型选择器收敛为**唯一**入口，不再「设置选择器」与「会话内
  config_options 模型下拉」两个并存。主模型触发器按当前 Agent 后端分流：
  - **Builtin（Calamex Agent）**：触发器驱动 `ai.json` 的 `selectedModel`，沿用阶段一/二的
    saveConfig 路径（主模型切换仅对 builtin 生效）。
  - **Kimi Code**：触发器直接驱动 Kimi 公示的 ACP `model` config_option，经
    `session/set_config_option` 实时热切换、零重启。
- **退役内存 override 与 dead path**：删除面板的 `selectedModelOverride` 内存态、以及
  `KIMI_MODES` / `kimiMode` defineModel / `modeSelectItems` / `handleModeSelect` 等
  「modes-作为-模型」的旧路径（对齐 ACP「config_options 取代 modes」的方向）。权限模式
  （Agent/Plan 等）那层 `modes` 保留。standalone config_options 下拉现只渲染**非模型**项
  （如思考强度），模型项被提升到主触发器。
- **已知局限（接受）**：若 Kimi 未在 ACP config_options 中公示 `model` 项，主选择器**锁定**
  并提示「Kimi 未公示可切换模型」——此时前端无法切换 Kimi 模型，需待 Kimi 端公示，或在
  设置里改走 moonshotai 厂商 + 填 Key 的 builtin 路径。
...
- 阶段三后前端只剩一个后端感知的模型选择器，去掉了重复下拉与 modes-as-model 死路径；
  Kimi 未公示模型时该选择器锁定并提示，属已接受的显式局限。
