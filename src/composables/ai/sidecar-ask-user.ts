 ...
 * 事件」与「续跑回灌请求」之间的双向投影收拢到一个纯函数模块，与驱动逻辑
 * （useAiAssistant 的原生回合段）解耦，便于单测。
 *
 * 关键事实（与已读源码一致，不自创）：
 ...
 * - 因此 ask_user 恢复镜像审批恢复（sidecarResolveApproval）的姊妹通道，
 *   以 outcome + 结构化 answers 取代审批的 decision 枚举（后者无法承载结构化答案）。
 ...
 * - sessionId 仅在非空时写入（实时预览订阅用；缺省由嬿主按 threadId 解析会话）。
 */
