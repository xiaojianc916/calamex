...
describe('resolvePersistedThreads（entries 唯一读路径优先级）', () => {
  it('新 key 严格解析成功 → 直接采用 entries', () => {
    ...
    const result = resolvePersistedThreads({ rawEntriesSnapshot: snapshot });
  ...
  it('新 key 缺失 → 空态', () => {
    const result = resolvePersistedThreads({ rawEntriesSnapshot: null });
    expect(result.source).toBe('empty');
    expect(result.activeThreadId).toBeNull();
    expect(result.threads).toEqual([]);
  });

  it('新 key 存在但不可救援（threads 非数组）→ 空态', () => {
  ...
  it('新 key 严格成功但 threads 为空 → 尊重空态', () => {
    const result = resolvePersistedThreads({
