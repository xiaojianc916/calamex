...
describe('AiPromptInput', () => {
  ...
  it('renders a session config option selector per option only for the Kimi agent', () => {
    ...
    expect(kimiWrapper.findAll('.ai-agent-trigger')).toHaveLength(1);
  });
  ...
  it('drives the Kimi model config option through the main model trigger', () => {
    ...
    const wrapper = mountPromptInput({ sessionConfigOptions });

    expect(wrapper.get('.ai-model-trigger__label').text()).toBe('Kimi K2');
    expect(wrapper.findAll('.ai-agent-trigger')).toHaveLength(0);
  });
