...
//
// 对话框 + 读取都下沉到 Rust 可信侧（services/ipc/attachment.service → pick_attachment_files）：
// 前端只传一个「初始目录提示」，拿不到、也传不进任何待读取路径——注入到 WebView
// 的脚本无法在用户未亲手选文件的情况下读任意文件。取代此前前端直连
// @tauri-apps/plugin-dialog + @tauri-apps/plugin-fs（后者需 fs:scope "**" 全盘读，属过度
// 授权，见地基审查 S1）。仅主目录回退仍用 @tauri-apps/api/path（非文件 I/O）。

import { pickAttachmentFiles } from '@/services/ipc/attachment.service';

const ATTACHMENT_LAST_DIR_KEY = 'calamex.ai.attachment.last-dir';

// 仅用于区分「图片 vs 文本」附件（附件管线只按 file.type 是否以 image/ 开头分类）。
// Tauri JS 侧无官方 MIME 推断能力，故保留这个极小映射。
...
// 记忆本次选中文件所在目录（后端返回 pickedDir），供下次作为对话框初始目录。
...
// Base64 → 字节：后端 pick_attachment_files 以 base64 回传文件内容，这里解码为 Uint8Array
// 供构造浏览器 File 对象（附件管线后续按 File 读文本 / 图片）。
...
export const pickAttachmentFilesViaNativeDialog = async (
  ...
  const defaultDir = await resolveDefaultDir(options.workspaceRootPath);
  const { files, pickedDir } = await pickAttachmentFiles(defaultDir ?? null);
  if (pickedDir) {
    rememberDir(pickedDir);
  }
  return files.map(
    (file) =>
      new File([decodeBase64ToBytes(file.base64)], file.name, { type: guessMimeType(file.name) }),
  );
};
