...
const placeholder = computed(() => (props.matchCase ? `${props.ariaLabel}（区分大小写）` : props.ariaLabel));
...
const applySuggestion = (suggestion: IWorkspaceEntrySuggestion): void => {
  ...
  draft.value = segments.map((segment, index) => (index === segments.length - 1 ? segment : segment.trim())).join(', ');
  closeSuggestions();
