...
const PRESERVED_ALIAS_LANGUAGE_IDS = new Set(['jsx', 'jsonc', 'less', 'scss', 'svg']);
...
const languageDescriptions: readonly LanguageDescription[] = [
  ...
  LanguageDescription.of({
    name: 'R',
    extensions: ['r'],
    load: streamLanguageLoader(() => import('@codemirror/legacy-modes/mode/r').then((m) => m.r)),
  }),
  LanguageDescription.of({
    name: 'Ruby',
    alias: ['rb'],
...
export function resolveCodeMirrorLanguageId(language: string): TCodeMirrorLanguageId {
  ...
  const description = LanguageDescription.matchLanguageName(languageDescriptions, normalized, true);
  if (!description) {
    return 'text';
  }
  ...
  return LANGUAGE_ID_BY_DESCRIPTION_NAME[description.name.toLowerCase()] ?? normalized;
}

const matchLanguageDescription = (languageId: string): LanguageDescription | null =>
