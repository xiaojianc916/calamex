 ...
 * services/tauri.ipc-metrics.ts 的单例口径保持一致。
 */
const UTF8_ENCODER = typeof TextEncoder !== 'undefined' ? new TextEncoder() : undefined;

/** 返回字符串的 UTF-8 字节长度；无 TextEncoder 的极小环境回退到 UTF-16 长度。 */
export const utf8ByteLength = (value: string): number =>
  value.length === 0 ? 0 : UTF8_ENCODER ? UTF8_ENCODER.encode(value).length : value.length;
