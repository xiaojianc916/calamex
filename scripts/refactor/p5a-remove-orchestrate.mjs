...
// 换行:Windows 工作树多为 CRLF;脚本读取时归一为 LF 以匹配锚点,写回时按原文件风格还原。
...
// ---- 换行归一助手:读为 LF,记住原风格,写回时还原 ----
function readLF(abs) {
  const raw = readFileSync(abs, "utf8");
  const crlf = raw.includes("\r\n");
  return { text: crlf ? raw.replace(/\r\n/g, "\n") : raw, crlf };
}
function writeKeepEol(abs, text, crlf) {
  writeFileSync(abs, crlf ? text.replace(/\n/g, "\r\n") : text, "utf8");
}
...
for (const { file, ops } of EDITS) {
  ...
  else staged.push({ abs, file, content, crlf });
}
...
if (DRY) {
  ...
  for (const s of staged) writeKeepEol(s.abs, s.content, s.crlf);
...
const selfNames = new Set(["2.mjs", "scripts/refactor/p5a-remove-orchestrate.mjs"]);
const hits = [];
for (const f of files) {
  const rel = relative(ROOT, f).split(sep).join("/");
  if (selfNames.has(rel)) continue;
