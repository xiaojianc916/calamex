...
const joinEnvValues = (values: Array<string>): string => values.filter(Boolean).join(path.delimiter);

const escapePowerShellString = (value: string): string => value.replace(/'/g, "''");

const runPowerShell = (script: string) => spawnSync(
    'powershell.exe',
...
`;
    ...
    const [summary] = parseJsonOutput(result.stdout) as Array<{ Id?: unknown; Name?: unknown; Path?: unknown }>;
    if (!summary) {
        return null;
    }
...
const normalizeInstallationPath = (value: string | undefined): string => value ? value.replace(/[\\/]+$/, '') : '';
...
const collectVisualStudioInstallations = (): VsInstallation[] => {
    ...
    const register = (installationPath: string | undefined, displayName = 'Visual Studio 2022'): void => {
