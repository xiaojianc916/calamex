...
//
// 用法：
//   node scripts/provision.mjs           # 缓存命中即跳过
//   node scripts/provision.mjs --force   # 强制全部重建
// 环境变量（国内加速，可选）：
...
function run(command, args, options = {}) {
  ...
  if (needsShell) {
    ...
    execSync([quote(command), ...args.map(quote)].join(' '), { stdio: 'inherit', ...options });
    return;
  }
...
async function provisionNode(lock) {
  ...
  if (!force && existsSync(dest) && lock.node?.exeSha256 && sha256File(dest) === lock.node.exeSha256) {
    log('Node 缓存命中，跳过');
    return;
  }
  ...
  let expected = lock.node?.zipSha256;
  if (!expected) {
...
function provisionShellcheck(lock) {
  ...
  if (lock.shellcheck?.sha256 && lock.shellcheck.sha256 !== sha) {
...
async function provisionShfmt(lock) {
  const dest = join(provisionRoot, 'bin', shfmtExeName);
  mkdirSync(dirname(dest), { recursive: true });
  const vend = join(vendorDir, shfmtExeName);
  if (existsSync(vend)) {
    const sha = sha256File(vend);
    if (lock.shfmt?.sha256 && lock.shfmt.sha256 !== sha) fail('shfmt 校验和与锁不一致');
    copyFileSync(vend, dest);
...
function provisionSidecar(manifest) {
...
function provisionLsp(manifest) {
...
main().catch((e) => fail(`未捕获异常：${e?.stack ?? e}`));