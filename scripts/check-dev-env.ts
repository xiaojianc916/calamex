...
if (pnpmVer) {
  ...
  fail(
    '\u627e\u4e0d\u5230 pnpm\uff0c\u8bf7\u8fd0\u884c: corepack enable && corepack prepare pnpm@latest --activate',
  );
}
...
if (!existsSync(cargoConfig)) {
  ...
  if (content.includes('replace-with') && content.includes('rsproxy')) {
    ...
    warn(
      '.cargo/config.toml \u5b58\u5728\uff0c\u4f46\u672a\u68c0\u6d4b\u5230 rsproxy \u955c\u50cf\u914d\u7f6e\uff0c\u9047\u5230\u4e0b\u8f7d\u95ee\u9898\u65f6\u8bf7\u68c0\u67e5\u8be5\u6587\u4ef6\u3002',
    );
  }
  if (content.includes('git-fetch-with-cli = true')) {
    ok(
      'git-fetch-with-cli = true\uff08\u53ef\u907f\u514d Windows schannel \u63e1\u624b\u95ee\u9898\uff09',
    );
  } else {
    warn(
      '\u672a\u8bbe\u7f6e git-fetch-with-cli = true\uff0c\u5728 Windows \u4e0b\u53ef\u80fd\u9047\u5230 TLS \u63e1\u624b\u9519\u8bef\u3002',
    );
  }
}
...
const cargoVer = cargoExecutable ? run(`"${cargoExecutable}" --version`) : run('cargo --version');
if (cargoVer) {
  ok(cargoVer);
} else {
...
if (hasError) {
  console.error(
    '[check-dev-env] \u73af\u5883\u68c0\u67e5\u53d1\u73b0\u9519\u8bef\uff0c\u8bf7\u5148\u4fee\u590d\u540e\u518d\u542f\u52a8\u3002\n',
  );
  process.exit(1);
} else {
